...
const CreateProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.attributeSize) initialAttributes.Size = formData.attributeSize;
            if (formData.attributeColor) initialAttributes.Color = formData.attributeColor;
            if (Object.keys(initialAttributes).length === 0) initialAttributes.Size = 'Standard';

            data.append('attributes', JSON.stringify(initialAttributes));
            images.forEach(img => data.append('images', img.file));
            const created = await handleCreateProduct(data);
            if (created && created._id) {
                navigate(`/seller/product/${created._id}`);
            } else {
        ...
        <>
            ...
            >
                <div className="max-w-6xl mx-auto px-8 lg:px-16 xl:px-24">
                    ...
                    {formError && (
                        <div className="mt-8 p-4 bg-red-50 border border-red-300 text-red-700 text-xs leading-relaxed font-sans">
                    ...
                    <form onSubmit={handleSubmit} className="pt-14 pb-24">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 lg:items-start">
                            ...
                            <div className="flex flex-col gap-12">
                                ...
                                {/* Initial Variant Details (Size, Color & Stock) */}
                                <div className="flex flex-col gap-3 border-t pt-6" style={{ borderColor: '#e4e2df' }}>
                                    <label className="text-[10px] uppercase tracking-[0.2em] font-medium" style={{ color: '#C9A96E' }}>
                                        Initial Variant Attributes (Size & Color)
                                    </label>
                                    <div className="grid grid-cols-3 gap-4">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <span className="text-[9px] uppercase tracking-[0.18em]" style={{ color: '#B5ADA3' }}>Initial Stock</span>
                                            <input
                                                type="number"
                                                name="stock"
