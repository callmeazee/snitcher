...
const SellerProductDetails = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fbf9f6] text-[#1b1c1a] font-sans pb-24">
      {/* Top Banner / Header */}
      <header className="sticky top-0 z-10 bg-[#fbf9f6]/80 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b border-[#e4e2df]">
        <h1 className="font-serif text-xl tracking-wide uppercase">{product.title?.substring(0, 20)}{product.title?.length > 20 ? '...' : ''}</h1>
        <button
          onClick={onLogout}
          className="text-xs uppercase tracking-[0.2em] font-medium text-[#7A6E63] hover:text-[#ba1a1a] transition-colors cursor-pointer"
        >
          Logout
        </button>
      </header>
